...
def split_and_save_dataset(
    ...
    if not samples:
        raise RuntimeError("未找到任何配对的img+dat样本，请检查源目录路径")
    ...
    print(f"数据保存至: {os.path.abspath(dst_root)}")
...
def train_val_model(
    ...
    print(f"预加载设备：{preload_device}")
    print(f"使用设备：{device} | 总训练轮数：{epochs} | 批次大小：{batch_size}")
    ...
    for epoch in range(epochs):
        ...
        train_metrics = {
            ...
            'r2': train_stats['total_r2'] / train_stats['num_samples'] # 新增
        }
        model.eval()
        val_stats = {
            'total_mse_all_coords': 0.0,
        ...
        val_metrics = {
            ...
            'r2': val_stats['total_r2'] / val_stats['num_samples'] # 新增
        }
        ...
        if current_metric < best_metric:
            ...
            print(f"📌 最佳模型更新：Epoch {best_epoch} | 最佳{monitor_metric}：{best_metric:.8f} | 最佳验证集R²：{best_val_r2:.8f}")
        else:
            patience_counter += 1
            print(
                f"⚠️  无提升累计：{patience_counter}/{patience} | 当前{monitor_metric}：{current_metric:.8f} | 最佳：{best_metric:.8f} | 当前验证集R²：{val_metrics['r2']:.8f}")

        if patience_counter >= patience:
            print(f"\n🚨 早停触发！连续{patience}个epoch监控指标未提升")
            print(f"最佳模型信息：Epoch {best_epoch} | 最佳{monitor_metric}：{best_metric:.6f} | 最佳验证集R²：{best_val_r2['r2']:.6f}")
        ...
        print(f"\n===== Epoch {epoch + 1}/{epochs} 指标汇总 =====")
        print(f"【训练集】")
        print(
            f"  单个坐标MSE：{train_metrics['avg_mse_per_coord']:.8f} | 单个坐标MAE：{train_metrics['avg_mae_per_coord']:.8f}")
        print(
            f"  单个样本MSE：{train_metrics['avg_mse_per_sample']:.8f} | 单个样本MAE：{train_metrics['avg_mae_per_sample']:.8f}")
        print(f"  R²（决定系数）：{train_metrics['r2']:.8f}")
        print(f"【验证集】")
        print(
            f"  单个坐标MSE：{val_metrics['avg_mse_per_coord']:.8f} | 单个坐标MAE：{val_metrics['avg_mae_per_coord']:.8f}")
        print(
            f"  单个样本MSE：{val_metrics['avg_mse_per_sample']:.8f} | 单个样本MAE：{val_metrics['avg_mae_per_sample']:.8f}")
        print(f"  R²（决定系数）：{val_metrics['r2']:.8f}")
        print("=" * 80 + "\n")
        scheduler.step()
    if save_metrics:
